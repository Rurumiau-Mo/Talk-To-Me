/* -----------------------------------------------------------------------------
     * TalkToMe speech tiles
     * -----------------------------------------------------------------------------
     *
     * This file creates and manages tiles that cause NPCs to speak.
     *
     * It writes:
     *   - TalkToMe flags, which are the source of truth.
     *   - Best-effort Monk's Active Tile Triggers flags.
     *   - Optional Tagger tags.
     *
     * TalkToMe also has a fallback movement listener in api.js/hooks.js so entry
     * triggers still work even if MATT's internal flag format changes.
     */
    ...
    /**
     * Get every TalkToMe-managed speech tile on the active scene.
     */
    ...
    /**
     * Check whether the centre of a token is inside a tile rectangle.
     */
    ...
    /**
     * Create best-effort Monk's Active Tile Triggers flag data.
     *
     * This may need adjustment for a specific MATT version, which is why TalkToMe
     * does not depend on these flags alone.
     */
    ...
    /**
     * Create a new TalkToMe speech tile on the current scene.
     */
    ...
    /**
     * Trigger a managed speech tile by tile id.
     */
    export async function triggerSpeechTile(api, tileId, tokenLike = null, overrides = {}) {
      ...
      // If the tile has a fixed NPC speaker, use that.
      // Otherwise, use the token which entered/triggered the tile.
