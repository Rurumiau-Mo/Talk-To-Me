  ...
  async handleEvent(clickType, event) {
    ...
    if (
      ...
      if (editableTile) {
        ...
        const app = game.talkToMe?.app;

        if (!app?.openTileEditor) {
          ui.notifications.error(
            "TalkToMe Tile Editor is not available."
          );
          return;
        }

        const opened = await app.openTileEditor(
          editableTile.id
        );

        if (!opened) {
          console.error(
            "TalkToMe double-click editor failed.",
            editableTile.id
          );
        }
