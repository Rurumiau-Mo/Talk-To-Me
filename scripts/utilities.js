...
export async function activateUtilityTemplate(tileDoc, tokenLike = null, overrides = {}) {
  ...
  if (utility.template === TTM_TEMPLATES.TRAP) {
    return utility.trapType === "magical"
      ? castSpellFromTile(tileDoc, tokenLike)
      : activateTrapTile(tileDoc, tokenLike);
  }
...
export async function runMultiUseActions(
  ...
  const runAction = async action => {
    ...
    if (template === TTM_TEMPLATES.TRAP) {
      if (action.trapType === "magical") {
        return castSpellFromTile(virtualTile, tokenLike);
      }
    ...
    if (template === TTM_TEMPLATES.MAGIC) {
      return castSpellFromTile(virtualTile, tokenLike);
    }
...
function getSpellActivities(spell) {
...
function getTokenCenter(tokenLike) {
  ...
  return {
    x: Number(
      ...
      token.center?.x !== undefined
        ? 0
        : Number(token.document?.width ?? 1)
          * Number(canvas.grid?.size ?? 100) / 2
    ),
    y: Number(
      token.center?.y
      ?? token.document?.y
...
function resolveMagicTemplateOrigin(
  tileDoc,
  caster,
  tokenLike,
  flags
) {
  const mode = flags.magicTemplateOrigin ?? "tile";

  if (mode === "caster") {
    return getTokenCenter(caster);
  }

  if (mode === "triggering-token") {
    return getTokenCenter(tokenLike);
  }

  if (mode === "custom") {
    const x = Number(flags.magicTemplateX);
    const y = Number(flags.magicTemplateY);
  ...
  return {
    x: Number(tileDoc.x ?? 0)
      + Number(tileDoc.width ?? 0) / 2,
    y: Number(tileDoc.y ?? 0)
      + Number(tileDoc.height ?? 0) / 2
  };
}

async function createMagicMeasuredTemplate(
  tileDoc,
  caster,
  tokenLike,
  flags
) {
  const type = flags.magicTemplateType ?? "none";
  if (type === "none") return null;

  const origin = resolveMagicTemplateOrigin(
    tileDoc,
    caster,
    tokenLike,
    flags
  ...
  const data = {
    ...
    distance: Math.max(
      0,
      Number(flags.magicTemplateDistance ?? 20)
    ),
    direction: Number(flags.magicTemplateDirection ?? 0),
    angle: Math.max(
      0,
      Number(flags.magicTemplateAngle ?? 90)
    ),
    width: Math.max(
      0,
      Number(flags.magicTemplateWidth ?? 5)
    ),
    user: game.user?.id,
  ...
  try {
    ...
    console.error(
      "TalkToMe failed to create the spell template.",
      error
    );
    ui.notifications.warn(
      "The spell cast, but its automatic template could not be created."
    );
    return null;
  }
}

function tokensInsideMeasuredTemplate(templateDoc) {
  if (!templateDoc) return [];

  const placeable =
    templateDoc.object
    ?? canvas.templates?.get?.(templateDoc.id)
    ?? null;
  const shape = placeable?.shape;
  const originX = Number(templateDoc.x ?? 0);
  const originY = Number(templateDoc.y ?? 0);
...
function resolveMagicTargets(
  ...
  const mode = flags.magicTargetMode ?? "none";
  ...
  if (mode === "tokens-within-tile") {
    return tokens.filter(token =>
      tileContainsToken(tileDoc, token.document ?? token)
    );
  }
  ...
  if (mode === "player-tokens") {
    return tokens.filter(token =>
      token.actor?.hasPlayerOwner === true
    );
  }
  ...
  if (mode === "template") {
    return tokensInsideMeasuredTemplate(templateDoc);
  }

  return [];
}

export async function castSpellFromTile(
  tileDoc,
  tokenLike = null
) {
  const flags = getUtilityFlags(tileDoc);
  const caster = canvas.tokens?.get(
    flags.magicCasterTokenId
  );
  const actor = caster?.actor;

  if (!actor) {
    ui.notifications.warn(
      "TalkToMe could not find the selected spellcaster."
    );
    return false;
  }

  const spell = actor.items?.get(
    flags.magicSpellItemId
  );

  if (!spell || spell.type !== "spell") {
    ui.notifications.warn(
      "TalkToMe could not find that spell on the caster."
    );
    return false;
  }

  const activities = getSpellActivities(spell);
  const activity = activities.find(item =>
    typeof item?.use === "function"
  ) ?? activities[0];

  const castLevel =
    flags.magicCastLevel === "auto"
      ? null
      : Number(flags.magicCastLevel);

  const autoCast =
    flags.magicAutoCast !== false
    && flags.magicConfigureDialog !== true;

  const useOptions = {
    configureDialog: autoCast
      ? false
      : flags.magicConfigureDialog === true
  };

  if (Number.isInteger(castLevel) && castLevel >= 0) {
    useOptions.spellLevel = castLevel;
    useOptions.level = castLevel;
  }

  if (flags.magicConsumeSlot === false) {
    useOptions.consume = false;
    useOptions.consumeSpellSlot = false;
  }

  const previousTargetIds = Array.from(
    game.user?.targets ?? []
  ).map(token => token.id);
  ...
  try {
    templateDoc = await createMagicMeasuredTemplate(
      tileDoc,
      caster,
      tokenLike,
      flags
    ...
    const targets = resolveMagicTargets(
      tileDoc,
      caster,
      tokenLike,
      templateDoc,
    ...
    if ((flags.magicTargetMode ?? "none") !== "none") {
      game.user?.updateTokenTargets?.(
        targets.map(token => token.id)
      );
    }

    if (activity && typeof activity.use === "function") {
      await activity.use(useOptions);
      return true;
    }

    if (typeof spell.use === "function") {
      await spell.use(useOptions);
      return true;
    }

    ui.notifications.warn(
      "This D&D5e spell does not expose a usable Activity."
    );
    return false;
  } finally {
    if ((flags.magicTargetMode ?? "none") !== "none") {
      game.user?.updateTokenTargets?.(previousTargetIds);
    }
  }
}
...
export async function applyUtilityTemplateActions(api, tileDoc, tokenLike = null) {
  ...
  if (flags.template === TTM_TEMPLATES.TRAP) {
    if (
      flags.trapTriggerOnce === true
      && flags.trapTriggeredOnce === true
    ) {
      return false;
    }
    ...
    const chance = Math.max(
      0,
      Math.min(100, Number(flags.trapTriggerChance ?? 100))
    );
    ...
    const delaySeconds = Math.max(
      0,
      Number(flags.trapDelaySeconds ?? 0)
    );
    if (delaySeconds > 0) {
      await new Promise(resolve =>
    ...
    if (flags.trapPauseGame === true && !game.paused) {
      await game.togglePause(true, true);
    }

    let result = true;

    if (flags.trapType === "magical") {
      result = await castSpellFromTile(tileDoc, tokenLike);
    } else {
    ...
    const updates = {};
    if (flags.trapTriggerOnce === true) {
      updates[`flags.${TTM_ID}.utility.trapTriggeredOnce`] = true;
    }
    if (flags.trapDisableAfterTrigger === true) {
      updates[`flags.${TTM_ID}.utility.disabled`] = true;
    }
  ...
  if (flags.template === TTM_TEMPLATES.MAGIC) {
    const result = await castSpellFromTile(tileDoc, tokenLike);
    return finishUtilityActivation(
      api,
      tileDoc,
      tokenLike,
...
async function resetTalkToMeTileToOriginalState(tileDoc) {
  ...
  for (const key of [
    ...
    "magicTemplateX",
    "magicTemplateY",
    "magicTemplateDistance",
    "magicTemplateAngle",
    "magicTemplateWidth",
    "magicTemplateDirection",
    "teleportX",
    "teleportY",
    "teleportOffsetX",
    "teleportOffsetY",
