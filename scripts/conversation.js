  ...
  try {
    for (const step of steps) {
      ...
      const remainingSteps =
        steps.length - step.sequenceIndex;
      const chainPanHoldDuration =
        Math.max(
          bubbleDurationMs,
          (remainingSteps - 1)
            * Math.max(lineGapMs, bubbleDurationMs)
            + bubbleDurationMs
        );
      ...
      await api.say({
        ...
        zoomToSpeaker:
          speech.zoomToSpeaker !== false,
        panHoldDuration: chainPanHoldDuration
      });
