...
export async function createSpeechTile({
  ...
  magicActivityId = "",
  magicCastLevel = "auto",
  magicConsumeSlot = true,
  magicAutoCast = true,
  magicConfigureDialog = false,
  magicTargetMode = "none",
  magicTemplateType = "none",
  magicTemplateOrigin = "tile",
  magicTemplateX = "",
  magicTemplateY = "",
  magicTemplateDistance = 20,
  magicTemplateAngle = 90,
  magicTemplateWidth = 5,
  magicTemplateDirection = 0,
  teleportSwitchX = "",
  teleportSwitchY = "",
  teleportX = "",
  teleportY = "",
  ...
  const flags = {
    [TTM_ID]: {
      ...
      utility: placementManager.addPlacementFlags({
        ...
        trapType,
        mundaneTrapType,
        trapDelaySeconds: Math.max(0, Number(trapDelaySeconds ?? 0)),
        trapTriggerChance: Math.max(
          0,
          Math.min(100, Number(trapTriggerChance ?? 100))
        ),
        trapTriggerOnce,
        trapDisableAfterTrigger,
        trapPauseGame,
        linkedTriggerTileId,
        moveDestinationX,
        moveDestinationY,
...
export async function triggerSpeechTile(api, tileId, tokenLike = null, overrides = {}) {
  ...
  await api.say({
    ...
    typingAnimation:
      overrides.typingAnimation ?? flags.typingAnimation
  });
